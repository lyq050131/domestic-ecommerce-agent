...
开发/测试依赖（运行 pytest 冒烟测试：配置/签名/存储/API mock）：
...
2. 浏览器打开 `http://127.0.0.1:8000/` 即运营后台单页。后台按「运营流程环节」分页显示：顶部环节导航（6 个环节 Tab）+「◀ 上一环节 / 下一环节 ▶」分页按钮，一次只展示当前环节内容，切换环节自动回到页首：
   - **环节1 📊 总览**：今日运营总览；
   - **环节2 📦 选品**：选品分析（填品类 → 开始分析约1-2分钟 → 展示真实商品数与 DeepSeek 报告）；
   - **环节3 📈 投放**：投放优化（一键生成推广优化方案约1分钟）+ 自动投放；
   - **环节4 💬 客服**：多语言客服（输入评论/私信 + 评分 → 生成回复）+ 客服待回复队列；
   - **环节5 🔄 沉淀复盘**：自反馈闭环统计（刷新查看知识库统计）+ 商品库 + 历史报告与趋势；
   - **环节6 ⚙️ 设置**：钉钉日报推送 + 运营设置。
   - **访问令牌（可选）**：`.env` 配置 `WEB_ACCESS_TOKEN` 后，打开后台需先登录（右上角「退出登录」可退出），API 调用需带 `Authorization: Bearer <令牌>`；
...
8. **今日运营总览**：页面顶部自动展示今日选品/投放摘要、商品库待办、近 7 天趋势与知识库状态，打开即见当天要点；
9. **商品库与状态流转**：推荐商品自动沉淀到「📦 商品库」（分页显示，每页 10/20/50/100 可调），可一键标记「待投放 / 已投放 / 效果待观察 / 已排除」，推荐不再是一次性结论；
10. **客服待回复队列**：「💬 客服待回复队列」可批量导入差评/私信，逐条或一键生成回复，标记已回复/忽略，贴近真实客服工作流；
11. **自动投放**：「🚀 自动投放」一键执行选品+投放分析 → 报告/商品落库 → 汇总推广链接清单 → 推送钉钉；采用**后台异步任务**执行（提交后前端轮询进度，不阻塞页面），推广链接清单按「选品 / 投放」两个环节分组展示、独立分页（每页 5/10/20 可调）；只生成可投放链接，不自动下单/发布（平台无此开放接口且属合规边界）；
12. **运营设置**：「⚙️ 运营设置」可页面化修改定时任务（开关/时刻/品类/数量）与钉钉开关/Webhook，写回 `.env` 后重启生效；
13. 接口文档：`http://127.0.0.1:8000/docs`（Swagger，可 Try it out 直接调试）；
14. 停止服务：双击 `stop_api.bat`。
...
> 本章 9.1–9.18 内联代码与交付目录源码逐字节一致（由脚本自动内联）。9.19 为 2026-08-14 新增模块（`storage/report_store.py` 报告落库、`api/scheduler.py` 每日定时任务）；9.20 为钉钉日报推送模块（`notify/dingtalk.py`）；9.21 为商品库模块（`storage/product_store.py`）；9.22 为客服待回复队列模块（`storage/cs_queue.py`）。运营后台前端 `api/static/index.html` 为静态页，未内联，以仓库源码为准。
...
### 9.17 api/app.py —— FastAPI 服务（入口：鉴权中间件 + 统一错误处理 + 路由挂载）

> 说明：v3.2 起按域拆分路由，业务接口在 `api/routers/` 下，请求模型在 `api/schemas.py`，长任务（自动投放）走 `api/task_manager.py` 后台线程执行、前端轮询进度。
...
### 9.17.1 api/schemas.py —— API 请求模型（Pydantic）
...
### 9.17.2 api/task_manager.py —— 后台任务队列（长任务异步化）
...
**`api/routers/system.py`** —— 系统状态与健康检查
...
**`api/routers/dashboard.py`** —— 今日运营总览与闭环统计
...
**`api/routers/cs.py`** —— 多语言客服：评论/私信回复 + 待回复队列
...
**`api/routers/products.py`** —— 商品库：列表 + 状态流转
...
**`api/routers/reports.py`** —— 历史报告：列表 / 趋势 / 详情
...
**`api/routers/settings.py`** —— 运营设置：读取 + 写入 .env
...
**`api/routers/launch.py`** —— 自动投放流水线（同步 + 异步任务）
...
DINGTALK_SECRET=
# 机器人「自定义关键词」安全设置的关键词（推送消息会自动带上；多个词以逗号分隔）
DINGTALK_KEYWORD=运营,电商

# ===== Web 后台访问令牌（可选） =====
# 留空=本机直连不鉴权；填写后访问 http://127.0.0.1:8000/ 需输入该令牌登录，API 需带 Authorization: Bearer <令牌>
WEB_ACCESS_TOKEN=
```
...
### 9.21 商品库与今日总览（storage/product_store.py）

> 2026-08-14 新增（P0）。报告推荐的 Top 商品自动沉淀为「商品池」，支持状态流转（待投放 / 已投放 / 已排除 / 效果待观察）；运营后台「今日运营总览」读取商品库待办与近 7 天趋势。
> 数据与报告库共用 `data/reports.db` 的 `products` 表；前端「📦 商品库」卡片负责筛选、状态标记与复制推广链接。

**`storage/product_store.py`**
...
### 9.22 客服待回复队列（storage/cs_queue.py）

> 2026-08-14 新增（P1）。批量导入差评/私信到待办队列，逐条或一键生成回复（DeepSeek），支持标记已回复/已忽略；运营后台「💬 客服待回复队列」卡片使用。
> 数据与报告库共用 `data/reports.db` 的 `customer_messages` 表；「⚙️ 运营设置」卡片可页面化修改定时任务与钉钉配置（写回 `.env`，重启生效）。
...
| POST | `/api/v1/cs/queue` | 客服队列批量导入 `{items:[{content,rating?}]}` |
| GET | `/api/v1/cs/queue?status=&limit=` | 客服队列列表（含统计） |
| POST | `/api/v1/cs/queue/{id}/reply` | 生成回复（调 DeepSeek） |
...
| GET | `/api/v1/dashboard/summary` | 今日运营总览（今日报告+商品库待办+趋势+知识库） |
| GET | `/api/v1/products?status=&source=&q=` | 商品库列表（含各状态统计） |
...
| POST | `/api/v1/auto/launch` | 自动投放流水线 `{category, top_n, push_dingtalk}` |
| POST | `/api/v1/notify/test` | 钉钉推送测试（验证 Webhook 配置） |
| GET | `/api/v1/system/status` | 系统状态（**不含任何密钥**，含定时任务开关） |
| GET | `/health` | 健康检查 |
| POST | `/api/v1/auth/login` | 登录 `{token}`（配置 WEB_ACCESS_TOKEN 后使用） |
| GET | `/api/v1/auth/status` | 登录状态（是否启用令牌） |
| POST | `/api/v1/auto/launch/async` | 自动投放（异步任务，返回 task_id，推荐） |
| GET | `/api/v1/tasks/{task_id}` | 查询后台任务状态（running/success/failed + progress + result） |

> 若 `.env` 配置了 `WEB_ACCESS_TOKEN`，所有 `/api/v1/*` 请求需带请求头 `Authorization: Bearer <令牌>`（`/api/v1/auth/*` 登录接口除外）。
...
线上环境推荐用 Docker 部署，一键拉起 Web 后台 + 每日定时任务：

```bash
# 服务器上执行（需 Docker + Compose）
git clone --depth 1 https://github.com/lyq050131/domestic-ecommerce-agent.git
cd domestic-ecommerce-agent
cp .env.example .env        # 填入 LLM_API_KEY / 淘宝三要素 / 钉钉 Webhook
export PIP_INDEX_URL=https://pypi.tuna.tsinghua.edu.cn/simple   # 国内加速（可选）
docker compose up -d --build
```

- 对外端口：8000（Web 后台 `/`、API 文档 `/docs`、健康检查 `/health`）
- 数据持久化：`data/`（报告库 reports.db、客服模板）、`logs/`、`storage/` 挂载宿主机目录
- 密钥注入：`.env` 通过 `env_file` 注入容器，**不打进镜像、不上传 GitHub**
- 访问令牌：设置 `WEB_ACCESS_TOKEN` 后 Web 需登录、API 需 `Authorization: Bearer <令牌>`
- 更新：`git pull && docker compose up -d --build`
- 完整步骤（安全组、Nginx + HTTPS、常见问题）见 `deploy/阿里云上线指南.md`
