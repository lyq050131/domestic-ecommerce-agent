...
7. 接口文档：`http://127.0.0.1:8000/docs`（Swagger，可 Try it out 直接调试）；
8. 停止服务：双击 `stop_api.bat`。
...
> 本章 9.1–9.18 内联代码与交付目录源码逐字节一致（由脚本自动内联）。9.17 已同步至含「报告落库 / 历史趋势 / 定时任务」的最新版；9.19 为 2026-08-14 新增模块（`storage/report_store.py` 报告落库、`api/scheduler.py` 每日定时任务）完整源码；运营后台前端 `api/static/index.html` 为静态页，未内联，以仓库源码为准。
