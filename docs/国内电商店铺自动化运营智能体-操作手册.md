...
4. 接口文档：`http://127.0.0.1:8000/docs`（Swagger，可 Try it out 直接调试）；
4. 停止服务：双击 `stop_api.bat`。
...
> 本章所有代码与交付目录 `domestic_ecommerce_agent_taobao/` 中的源码**逐字节一致**（由脚本自动内联），保证手册即代码、代码即手册。
...
| GET | `/api/v1/system/status` | 系统状态（**不含任何密钥**） |
| GET | `/health` | 健康检查 |
