...
10. 接口文档：`http://127.0.0.1:8000/docs`（Swagger，可 Try it out 直接调试）；
11. 停止服务：双击 `stop_api.bat`。
...
> 本章 9.1–9.18 内联代码与交付目录源码逐字节一致（由脚本自动内联）。9.19 为 2026-08-14 新增模块（`storage/report_store.py` 报告落库、`api/scheduler.py` 每日定时任务）；9.20 为钉钉日报推送模块（`notify/dingtalk.py`）；9.21 为商品库模块（`storage/product_store.py`）。运营后台前端 `api/static/index.html` 为静态页，未内联，以仓库源码为准。
